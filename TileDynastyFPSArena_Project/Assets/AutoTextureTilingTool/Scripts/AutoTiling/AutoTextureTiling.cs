...
namespace AutoTiling {
    ...
    public class AutoTextureTiling : MonoBehaviour {
        ...
        [SerializeField]
        private UnwrapType _unwrapMethod = UnwrapType.CubeProjection;
        ...
        public Rect GetFaceBounds(FaceData face) {
            ...
            Quaternion rotationToTop = Quaternion.FromToRotation(Vector3.Scale(face.AverageNormal, transform.localScale), Vector3.up);
            for (int t = 0; t < face.Triangles.Length; t++) {
                Vector3 vector = meshFilter.sharedMesh.vertices[face.Triangles[t]];
                Vector3 rotatedVector = rotationToTop * new Vector3(vector.x * transform.localScale.x, vector.y * transform.localScale.y, vector.z * transform.localScale.z);
...
#if UNITY_EDITOR
            Mesh meshToEdit = meshFilter.sharedMesh;
#else
        ...
        private MeshData SplitMeshForFaceUnwrapping(MeshData meshData) {
            ...
            for (int m = 0; m < meshData.Triangles.Length; m++) {
                for (int i = 0; i < meshData.Triangles[m].Count; i += 3) {
                    ...
                    triangleNormal /= 3f; 
                    triangleNormal = new Vector3(triangleNormal.x / transform.localScale.x, triangleNormal.y / transform.localScale.y, triangleNormal.z / transform.localScale.z).normalized;
        ...
        private Vector2 VerticeUVByFace(Vector3 vertex, FaceData faceData) {
            ...
            Vector3 normal = Vector3.Scale(faceData.AverageNormal, transform.localScale);
            Vector3 localVertexCoord = Quaternion.FromToRotation(normal, Vector3.up) * new Vector3(vertex.x * transform.localScale.x, vertex.y * transform.localScale.y, vertex.z * transform.localScale.z);
        ...
        public void OnDrawGizmosSelected() {
            ...
            //    startPos = transform.position + transform.rotation * new Vector3(transform.localScale.x * startPos.x, transform.localScale.y * startPos.y, transform.localScale.z * startPos.z);
            ...
            //        GL.Vertex(transform.rotation * (new Vector3(currentVertex.x * transform.localScale.x, currentVertex.y * transform.localScale.y, currentVertex.z * transform.localScale.z)) + transform.position);
            //    }
            //    GL.End();
            //    GL.PopMatrix();
            //}
                    ...
                    for (int j = 0; j < 3; j++) {
                        ...
                        if (index < _meshFilter.sharedMesh.vertices.Length) {
                            ...
                            averageFacePosition += (transform.rotation *
                                new Vector3(newPos.x * transform.localScale.x, newPos.y * transform.localScale.y, newPos.z * transform.localScale.z));
            ...
            //    }
            //    averageFacePosition = (averageFacePosition / (faceUnwrapData[i].Triangles.Length)) + transform.position;
            //    Vector3 normal = new Vector3(faceUnwrapData[i].AverageNormal.x / transform.localScale.x, faceUnwrapData[i].AverageNormal.y / transform.localScale.y, faceUnwrapData[i].AverageNormal.z / transform.localScale.z).normalized;
